...
params <- c("int.gs","line.sd","site.sd","year.sd","line.year.sd","site.year.sd",
            "expGroupSize","predGroupSize","random.gs.line.year","out2")
...
allCombos<-expand.grid(Line=unique(allDetections$LinjeID),Year=unique(allDetections$Year))
...
for(i in 1:nrow(allDetections)){
  TransYrIdx[i,which(allCombos$Year==allDetections$Year[i]&
    allCombos$Line==allDetections$LinjeID[i])]<-1
}
