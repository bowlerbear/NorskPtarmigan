...
#clean columns
ROSData$temp <- as.numeric(ROSData$temp)
ROSData$precip <- as.numeric(ROSData$precip)
...
#now count number of days above 0 amd precip above 0 each year and site
outAllsubset<-subset(outAll, temp>0 & precip>0)
out<-table(outAllsubset$site,outAllsubset$Year)
outM<-melt(out)
names(outM)<-c("site","Year","ROS")
