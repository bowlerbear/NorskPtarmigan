...
```
#plotting the data by temperature

```{r}
#get total indivd per line per year
totalIndiv_RY <- ddply(allData,.(Year,LinjeID,Rapporteringsniva),summarise,totalIndiv=sum(totalIndiv),
                     SettSmagnager=max(SettSmagnager,na.rm=T),transectLength=max(LengdeTaksert,na.rm=T))

totalIndiv_RY <- ddply(totalIndiv_RY,.(Year,Rapporteringsniva),summarise,
                       totalIndiv=mean(totalIndiv/transectLength,na.rm=T),
                       rodentprop=mean(SettSmagnager[!is.na(SettSmagnager)&!is.infinite(SettSmagnager)]))

#get average temp per rapp per year
...
winterData$Rapporteringsniva<-allData$Rapporteringsniva[match(winterData$site,allData$LinjeID)]
winterData_RY <- ddply(winterData,.(Rapporteringsniva),summarise,temp=mean(temp))
totalIndiv_RY <- merge(totalIndiv_RY,winterData_RY,by=c("Rapporteringsniva"))

#raw graph
qplot(rodentprop,totalIndiv,data=totalIndiv_RY,colour=temp)+
  scale_colour_gradient(low="steelblue",high="red")

#cut temp into a factor
totalIndiv_RY$tempF<-cut_number(totalIndiv_RY$temp,n=2)
qplot(asin(sqrt(rodentprop)),totalIndiv,data=totalIndiv_RY)+
  facet_wrap(~tempF)+
