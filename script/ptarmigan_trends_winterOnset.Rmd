...
setwd("/data/home/diana.bowler/snowData")
library(lubridate)
...
winterData <- ldply(tempFiles, function(x){
      ...
      temp <- subset(temp,month%in%c(9:12))
      temp$depth <- as.numeric(temp$depth)
      subset(temp, depth>0)
})

#get average per site/lineid 725 ids
winterData$Year <- year(winterData$Date)
winterData <- ddply(winterData, .(week,Year,site), summarise, snowDays = length(depth))
winterData <- subset(winterData, snowDays==7)
winterData <- ddply(winterData, .(Year,site), summarise, onset = min(week))
