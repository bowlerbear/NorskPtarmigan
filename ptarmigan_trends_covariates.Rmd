...
all(myCovariate$OmradeID%in%rowNames)
all(myCovariate$Year%in%dimnames(covariateMatrix)[[2]])
...
harvestdata$Year<-as.numeric(sapply(harvestdata$Year,function(x)strsplit(as.character(x),"\\.")[[1]][1]))
...
#get fylke area data
countyArea <- read.delim("RSF_10bin_by county.txt",as.is=T)
countyArea$county[which(countyArea$county=="Nord-Tr\xf8ndelag")]<-"Nord-Trøndelag" 
countyArea$county[which(countyArea$county=="S\xf8r-Tr\xf8ndelag")]<-"Sør-Trøndelag" 
countyArea$county[which(countyArea$county=="\xd8stfold")]<-"Østfold" 
countyArea <- ddply(countyArea,.(county), summarise, area = sum(RSF.km2[RSFbin!=1]))
unique(countyArea$county)
unique(harvestdata$Fylke)

harvestdata <- merge(harvestdata,countyArea, by.x="Fylke", by.y = "county")
qplot(Year,harvestBag/area,data=subset(harvestdata,Year>=2006&Fylke%in%nuIndivs$Fylkesnavn),colour=Fylke,geom=c("point","line"))
...
library(reshape2)
outMelt<-melt(out,id=c("Line","Fylkesnavn","Rapporteringsniva","LinjeID"))
names(outMelt)[5:6]<-c("Year","Density")
outMelt$Fylkesnavn<-sapply(outMelt$Line,function(x)strsplit(as.character(x),"_")[[1]][1])
outMelt$Year<-as.numeric(gsub("X","",outMelt$Year))
outMelt$Year<-outMelt$Year + 2006

#average density per fylk
densityFylke<-ddply(outMelt,.(Fylkesnavn,Year),summarise,meanDensity=mean(Density))

#merge with harvest bag data
densityFylke<-merge(densityFylke,harvestdata,by.x=c("Year","Fylkesnavn"),by.y=c("Year","Fylke"))

qplot((harvestBag/area),meanDensity,data=densityFylke,
      color=Fylkesnavn,geom=c("point"))+stat_smooth(method="lm",se=F)

qplot(meanDensity,(harvestBag/area),data=densityFylke,
      color=Fylkesnavn,geom=c("point"))+stat_smooth(method="lm",se=F)

qplot(Year,(harvestBag/area)/meanDensity,data=densityFylke,colour=Fylkesnavn,
      geom=c("point"))+stat_smooth(method="lm",se=F)

qplot(meanDensity,(harvestBag/area)/meanDensity,data=densityFylke,colour=Fylkesnavn,
      geom=c("point"))+stat_smooth(method="lm",se=F)

```
```{r}
#plot annual trends
outMelt$Rapport<-sapply(outMelt$Line,function(x)strsplit(as.character(x),"#")[[1]][1])

ggplot(outMelt,aes(x=Year,y=Density,group=LinjeID))+
  geom_line()+
  geom_point(aes(colour=factor(Rapport)))+
